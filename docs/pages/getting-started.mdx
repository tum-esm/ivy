...
Make sure that all tests are passing. In the GitHub Actions CI, everything is already set up, so we recommend to use it. You can still run the tests locally though.
...
For these updater tests to work, your Ivy root directory (`~/Documents/myivy`) has to be empty to ensure that nothing is overwritten – this is why we recommend just running these tests in the CI environment. The updater test will set up the current setup twice under `~/Documents/myivy/myivy/1.2.3` and `~/Documents/myivy/myivy/4.5.6`, run the first version regularly and try to update it to the second version. This proves that the current codebase can be updated *from* and *to*.
...
**1. Set up a the computer running the DAS (e.g. a Raspberry Pi)**
...
This will restart the DAS after any crash or system reboot, or after any software update or configuration change of your DAS.
