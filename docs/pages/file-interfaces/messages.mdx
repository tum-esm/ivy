...
However, you do not have to manually send out log messages because the logger already does that (see `config.logging_verbosity`):
...
In case the backend is offline for a while or the connection between MQTT broker and server breaks, the messages are stored locally in `data/messages/`. There is one file `YYYY-MM-DD.csv"` per day and each line can be parsed using the schema below. The following script does just that:
