...
The term **"foreign config"** describes a configuration object that belongs to a different version of the automation software. This distinction exists, because the configuration schema can radically change between different software versions. Only a few fields (like version number) have to be present in any version you want to update this software to.
