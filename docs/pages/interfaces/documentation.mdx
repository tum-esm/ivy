...
The interfaces should not change between patch versions (e.g., `1.0.0` to `1.0.1` or `0.1.5` to `0.1.6`), that is why there should be one documentation per minor version (e.g., `0.1`, `0.2`, `1.0`, `1.1`, ...). This template includes a disabled GitHub Action that will check out a branch named `docs-vX.Y` for every tag you push to GitHub. When you push the tags `v0.1.0`, then `v0.1.1`, then `v0.1.2` and so on, the action will keep the respective `docs-v0.1` branch up to date.
