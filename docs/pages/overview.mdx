...
Ivy/your DAS is set up at `~/Documents/ivy/x.y.z/`. The full layout of the file system is described [here](/concepts/file-system-layout). It is configured using a JSON configuration file at `~/Documents/ivy/x.y.z/config/config.json`. Read more about the configuration object [here](/interfaces/configuration).
