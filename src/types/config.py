...
class _LoggingVerbosityConfig(pydantic.BaseModel):
    ...
    file_archive: Literal["DEBUG", "INFO", "WARNING", "ERROR", "EXCEPTION", None] = pydantic.Field(
        default=..., description="The minimum log level for the file archive in `data/logs`"
    )
    console_prints: Literal["DEBUG", "INFO", "WARNING", "ERROR", "EXCEPTION", None] = (
        pydantic.Field(
            default=...,
            description="The minimum log level for the console prints",
        )
    )
    message_sending: Literal["DEBUG", "INFO", "WARNING", "ERROR", "EXCEPTION", None] = (
        pydantic.Field(default=..., description="The minimum log level for the message sending")
    )
...
class MQTTBrokerConfig(pydantic.BaseModel):
    host: str = pydantic.Field(
        ..., min_length=1, max_length=512, description="The host to use for the MQTT connection"
    )
